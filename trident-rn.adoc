...
* Aggiunto link:https://docs.netapp.com/us-en/trident/trident-reco/security-reco.html#enable-per-volume-host-side-encryption-using-linux-unified-key-setup-luks["Supporto per volumi con crittografia LUKS"] per `ontap-san` e. `ontap-san-economy` driver di storage.
