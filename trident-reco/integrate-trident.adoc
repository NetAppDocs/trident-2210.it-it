...
=== Cloud Volumes Service con driver backend GCP
...
. *CVS-Performance*: Tipo di servizio ottimizzato per le performance più adatto per i carichi di lavoro di produzione che apprezzano le performance. Scegli tra tre livelli di servizio unici [`standard`, `premium`, e. `extreme`].


Le dimensioni minime dei volumi CVS e CVS-Performance sono 100 GiB.
...
== Progettazione di Virtual Storage Pool
